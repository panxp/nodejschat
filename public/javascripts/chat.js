$(document).ready(function() {
  ...
  var name = GetQueryString("nickname"),
    room = GetQueryString("room"); //从 cookie 中读取用户名，存于变量 from
  ...
  socket.on('online', function(data) {
    ...
    //刷新用户在线列表
    flushUsers(data.users, data.room);
    //显示正在对谁说话
    showSayTo();
  });
  ...
  //刷新用户在线列表
  function flushUsers(users, currentRoom) {
    //清空之前用户列表，添加 "所有人" 选项并默认为灰色选中效果
    $("#list").empty().append('<li title="双击聊天" alt="all" class="sayingto" onselectstart="return false">所有人</li>');

    //遍历生成用户在线列表
    for (var i in users) {
     // if (parseInt(u[1]) == currentRoom) {
        $("#list").append('<li alt="' + users[i].nickname + '" title="双击聊天" onselectstart="return false">' + users[i].nickname + '</li>');
     // }
